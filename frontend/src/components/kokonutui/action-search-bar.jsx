...
function ActionSearchBar({
  ...
  useEffect(() => {
    if (isFocused && inputWrapRef.current) {
          ...
          <AnimatePresence>
            {isFocused && result && !selectedAction && (
              <motion.div
                ...
                className="overflow-hidden rounded-md border bg-white shadow-xs dark:border-gray-800 dark:bg-black"
                exit="exit"
                initial="hidden"
                role="listbox"
                ...
                <motion.ul role="none">
                  {result.actions.map((action) => (
                    <motion.li
                      ...
                      className={`flex cursor-pointer items-center justify-between rounded-md px-3 py-2 hover:bg-gray-200 dark:hover:bg-zinc-900 ${activeIndex === result.actions.indexOf(action)
